    ...
    filtered_guild_members.each do |member|
      ...
      while counter < 10
        unless character_info.nil?
          ...
          item_keys = %w[head neck shoulder back chest wrist hands waist legs feet finger1 finger2 trinket1 trinket2 mainHand offHand]
          item_keys.each do |key|
            if character_info['items'].has_key?(key)
              value = character_info['items'][key]
              value['description'] = ::Api::Request.getItemInfo(value['id'])

              if value['tooltipParams'].length
                if value['tooltipParams'].has_key?('transmogItem')
                  value['transmog'] = ::Api::Request.getItemInfo(value['tooltipParams']['transmogItem'])
                end

                gemsArr = []
                for i in 0...value['tooltipParams'].length
                  if value['tooltipParams'].has_key?("gem#{i}")
                    gemsArr.push(::Api::Request.getItemInfo(value['tooltipParams']["gem#{i}"]))
                  end
                end
                value["gems"] = gemsArr
              end
